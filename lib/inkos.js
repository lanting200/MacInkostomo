...
import { runPublisherInkOS } from './inkos-runtime.js';
...
const INKOS_FRAMEWORK_EVENT_PREFIX = '@@INKOS_EVENT@@';
  ...
  return new Promise((resolve, reject) => {
    ...
    const logChunk = (stream, chunk) => {
      ...
      for (const line of rawText.split(/\r?\n/)) {
        const markerAt = line.indexOf(INKOS_FRAMEWORK_EVENT_PREFIX);
        if (markerAt < 0) {
          visibleLines.push(line);
          continue;
        }
        try {
          ingestDebugEvent(JSON.parse(line.slice(markerAt + INKOS_FRAMEWORK_EVENT_PREFIX.length)), {
          ...
          debugEvent('inkos.runtime', 'invalid streaming framework event', {
            error: error.message,
      ...
      if (trimmed && stream === 'stderr') {
        for (const line of trimmed.split(/\r?\n/).map(value => value.trim()).filter(Boolean).slice(-200)) {
          const isError = /^(?:ERROR|FATAL)|\berror\b|失败|异常/i.test(line);
          const isWarning = /^(?:WARN|WARNING)|\bwarn(?:ing)?\b|警告/i.test(line);
          debugEvent('inkos.log', line.slice(0, 8_000), { label }, isError ? 'error' : isWarning ? 'warn' : 'info', {
            traceId: opts.traceId,
            operation: 'inkos.log',
...
export async function reviseChapter(bookId, chapterNum, chapterTitle, revisionNote, chapterContent, preferredMode = 'auto', opts = {}) {
  ...
  // Normal Publisher revisions stay inside the source-integrated framework.
  // Agent/interact are session tools and remain an explicit compatibility path.
  if (process.env.PUBLISHER_INKOS_RUNTIME !== 'subprocess'
    && requestedInProcessRevisionMode(preferredMode)) {
    try {
      const result = await runPublisherInkOS('write.revise', bookId, {
        chapterNumber: chapterNum,
        mode: preferredMode === 'auto' ? undefined : preferredMode,
        externalContext: brief,
        traceId: opts.traceId,
        timeoutMs: Number(process.env.PUBLISHER_REVISE_TIMEOUT_MS || '2700000'),
        callbacks: {
          onTextDelta: opts.onTextDelta,
          onStreamProgress: (progress) => opts.onProgress?.({
            stage: 'stream',
            eventKey: `stream:${progress.status}`,
            label: `模型输出 ${Math.round(progress.elapsedMs / 1000)}s`,
            detail: `${progress.totalChars} chars`,
          }),
        },
      });
      const newContent = reReadChapter(bookId, chapterNum);
      if (result?.applied && newContent !== null && newContent !== chapterContent) {
        return {
          success: true,
          response: result,
          newContent,
          reviseMode: preferredMode === 'auto' ? 'spot-fix' : preferredMode,
          runtime: 'framework',
      ...
      return {
        ...
        noChange: true,
        response: result,
        error: result?.skippedReason || '内置 InkOS 修订未产生正文变化',
        reviseMode: preferredMode,
        runtime: 'framework',
        knowledgeInjected: knowledge ? knowledge.length : 0,
      };
    } catch (error) {
      if (!isFrameworkUnavailableError(error)) {
        return {
          success: false,
          error: `内置 InkOS 修订失败: ${error.message}`,
          runtime: 'framework',
          knowledgeInjected: knowledge ? knowledge.length : 0,
        };
      }
      console.warn(`[reviseChapter] 内置 core 未就绪，回退 CLI：${error.message}`);
    }
  }
 ...
 *
 * LIMITED TO THE LAST CHAPTER: the embedded module replays truth state from
 * snapshot N-1 and replaces chapter N. The route keeps the last-chapter gate
 * because later chapters would need a complete causal replay after any full
 * rewrite. The CLI path below remains a compatibility fallback.
 *
...
export async function rewriteChapter(bookId, chapterNum, chapterTitle, brief, chapterContent, prevTail = null, nextHead = null) {
  ...
  if (process.env.PUBLISHER_INKOS_RUNTIME !== 'subprocess') {
    try {
      const response = await runPublisherInkOS('write.rewrite', bookId, {
        chapterNumber: chapterNum,
        externalContext: briefText,
        timeoutMs: Number(process.env.PUBLISHER_REWRITE_TIMEOUT_MS || '600000'),
      });
      const newContent = reReadChapter(bookId, chapterNum);
      if (newContent === null) {
        return {
          ...
          response,
          runtime: 'framework',
          error: '内置 InkOS 已执行，但章节正文重读失败',
          knowledgeInjected: knowledge ? knowledge.length : 0,
        };
      }
      if (!response?.applied || newContent === chapterContent) {
        return {
          success: false,
          noChange: true,
          response,
          runtime: 'framework',
          error: response?.skippedReason || '内置 InkOS 重写未产生正文变化',
          knowledgeInjected: knowledge ? knowledge.length : 0,
        };
      }
      return {
      ...
      if (!isFrameworkUnavailableError(err)) {
        return {
          success: false,
          error: err.message,
          runtime: 'framework',
      ...
      console.warn(`[rewriteChapter] 内置 core 未就绪，回退 CLI：${err.message}`);
    }
  }
...
function requestedInProcessRevisionMode(mode) {
  return !['agent', 'interact'].includes(String(mode || '').toLowerCase());
}

function isFrameworkUnavailableError(error) {
  const message = String(error?.message || error || '');
  return /内置 InkOS core 未构建|Cannot find module|ERR_MODULE_NOT_FOUND|core 未就绪/i.test(message);
}
...
export async function generateChapter(bookId, guidance, opts = {}) {
  ...
  if (mode !== 'draft' && process.env.PUBLISHER_INKOS_RUNTIME !== 'subprocess') {
    try {
      const result = await runPublisherInkOS('write.next', bookId, {
        wordCount: opts.targetWords,
        externalContext: guidance,
        traceId: opts.traceId,
        timeoutMs: Number(process.env.PUBLISHER_GENERATE_TIMEOUT_MS || process.env.PUBLISHER_DRAFT_TIMEOUT_MS || '2700000'),
        callbacks: {
          onTextDelta: opts.onTextDelta,
          onStreamProgress: (progress) => opts.onProgress?.({
            stage: 'stream',
      ...
      const number = Number(result?.chapterNumber);
      const entry = readChapterIndex(bookId, { strict: true }).find(chapter => chapter.number === number);
      const newContent = Number.isSafeInteger(number) ? reReadChapter(bookId, number) : null;
      if (result && Number.isSafeInteger(number) && newContent !== null) {
        const title = result.title || entry?.title || '';
        try { backfillChapterSummary(bookId, number, title, newContent); } catch (error) {
          console.warn(`[generateChapter] story-memory 补写异常(忽略): ${error.message}`);
        }
        return {
          success: true,
      ...
      return { success: false, error: '内置 InkOS 未写入可读取的新章节', response: result, runtime: 'framework' };
    } catch (error) {
      if (!isFrameworkUnavailableError(error)) {
        return { success: false, error: `内置 InkOS 生成失败: ${error.message}`, runtime: 'framework' };
      }
      console.warn(`[generateChapter] 内置 core 未就绪，回退 CLI：${error.message}`);
    }
  }
  ...
  const requestedWords = opts.targetWords
    ?? process.env.PUBLISHER_GENERATE_WORDS
    ?? process.env.PUBLISHER_DRAFT_WORDS;
  if (requestedWords !== undefined && requestedWords !== null && requestedWords !== '') {
    const words = Number(requestedWords);
    if (!Number.isSafeInteger(words) || words < 1) {
      throw new Error(`无效的章节目标字数: ${String(requestedWords)}`);
    }
    args.push('--words', String(words));
  }
    ...
    try {
      ...
      const result = await runInkosStreaming(commandArgs, {
        ...
        env: {
          ...
          ...(opts.traceId ? { MACINKOSTOMO_TRACE_ID: opts.traceId } : {}),
        ...
        traceId: opts.traceId,
        bookId,
        chapterNumber: oldIndex.length + 1,
        onOutput: ({ text }) => {
          reportProgress.push(text);
          reportWriterText.push(text);
        },
