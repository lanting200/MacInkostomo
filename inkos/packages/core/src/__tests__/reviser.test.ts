...
describe("ReviserAgent", () => {
  ...
  it("tells the model that length is advisory when a length spec is provided", async () => {
    ...
    try {
      ...
      expect(systemPrompt).toContain("字数仅作参考");
      expect(systemPrompt).toContain("不要为了凑字数而删减正文");
      expect(systemPrompt).toContain("=== PATCHES ===");
      expect(systemPrompt).not.toContain("=== REVISED_CONTENT ===");
      expect(userPrompt).toContain("字数仅作编辑参考");
      expect(userPrompt).toContain("不要为压缩篇幅删掉完整段落");
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
