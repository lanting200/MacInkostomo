...
You MUST emit all **5 SECTION blocks in order**: story_frame → volume_map → roles → book_rules → pending_hooks. Do NOT stop after story_frame or volume_map just because they ran long. Even if roles lists only 3 characters, book_rules is a small Markdown block, and pending_hooks has only 3 rows, all five must appear. The output is only considered delivered after the last row of pending_hooks is written.`;
    ...
    try {
      ...
      try {
        ...
        if (repairError instanceof MissingArchitectSectionsError) {
          const completed = this.completeRecoverableMissingSections(repairError, language);
          if (completed) {
            return this.parseSections(completed, language);
          }
  ...
  private completeRecoverableMissingSections(
    error: MissingArchitectSectionsError,
    language: "zh" | "en",
  ): string | null {
    const recoverable = new Set(["book_rules", "pending_hooks"]);
    if (!error.missing.every((name) => recoverable.has(name))) {
      return null;
    }

    const parsed = this.parseArchitectSectionMap(error.content);
    const hasFoundation = Boolean(parsed.get("story_frame") || parsed.get("story_bible"))
      && Boolean(parsed.get("volume_map") || parsed.get("volume_outline"))
      && Boolean(parsed.get("roles"));
    if (!hasFoundation) return null;

    const blocks: string[] = [error.content.trimEnd()];
    if (error.missing.includes("book_rules")) {
      blocks.push(language === "en"
        ? `=== SECTION: book_rules ===
- Keep the protagonist objective, world rules, character arcs, and volume map consistent.
- Every chapter must advance at least one visible plot pressure, relationship change, clue, or cost.
- Do not contradict established role cards, the story frame, or the volume map.
- Maintain platform pacing: strong hook, clear conflict, short-term payoff, and a new question at the end of each chapter.
- This section was completed by the deterministic fallback because the model omitted book_rules after producing the core foundation.`
        : `=== SECTION: book_rules ===
- 严格保持主角目标、世界规则、角色弧线与分卷地图一致。
- 每章必须推进至少一个可见的剧情压力、关系变化、线索或代价。
- 不得违背已生成的角色卡、story_frame 与 volume_map。
- 保持平台节奏：强钩子、明确冲突、阶段爽点、章末新问题。
- 本段由确定性兜底补全：模型已生成核心基础设定，但遗漏了 book_rules。`);
    }
    if (error.missing.includes("pending_hooks")) {
      blocks.push(language === "en"
        ? `=== SECTION: pending_hooks ===
| hook_id | start_chapter | type | status | last_advanced_chapter | expected_payoff | payoff_timing | depends_on | pays_off_in_arc | core_hook | half_life | notes |
|---|---:|---|---|---:|---|---|---|---|---|---:|---|
| hook-main-1 | 0 | mainline | open | 0 | Pay off the Book Objective through the staged volume KRs. | across the full book |  | volume arcs | true | 30 | Deterministic fallback seed because the model omitted pending_hooks. Expand during chapter planning. |`
        : `=== SECTION: pending_hooks ===
| hook_id | start_chapter | type | status | last_advanced_chapter | expected_payoff | payoff_timing | depends_on | pays_off_in_arc | core_hook | half_life | notes |
|---|---:|---|---|---:|---|---|---|---|---|---:|---|
| hook-main-1 | 0 | 主线 | open | 0 | 围绕 Book Objective 与各卷 KR 分阶段回收。 | 贯穿全书 |  | 各卷主线弧 | true | 30 | 模型遗漏 pending_hooks 后由确定性兜底生成；后续章节规划时继续扩展。 |`);
    }
    return blocks.join("\n\n");
  }
  ...
  private parseArchitectSectionMap(content: string): Map<string, string> {
    ...
    const markerMatches = [...content.matchAll(sectionPattern)].map((match) => ({
      name: this.canonicalSectionMarkerName(match[1] ?? ""),
      index: match.index ?? 0,
...
${trimmed}\n`;
  ...
  private canonicalSectionMarkerName(name: string): string {
    const normalized = this.normalizeSectionName(name);
    const raw = name.normalize("NFKC").trim();

    // Explicit legacy SECTION markers retain their legacy identity so the
    // parser can apply its documented roles-less compatibility path.
    if (normalized === "story_bible" || /^(?:故事圣经|故事聖經)$/.test(raw)) {
      return "story_bible";
    }
    if (normalized === "volume_outline" || /^(?:卷纲|卷綱|分卷大纲|分卷大綱)$/.test(raw)) {
      return "volume_outline";
    }

    return this.canonicalSectionNameFromHeading(name) ?? normalized;
  }
