...
- 所有会跨章再次出现、承载身份/线索/伏笔、或被角色持续持有的实体物品，必须写入 RUNTIME_STATE_DELTA.objectOps.upsert
- 为同一件物品使用稳定 objectId；名称变化只能增加 aliases，不能另建一个“看似相似”的新对象
- material（材质）和 inscription（刻字/纹样）属于稳定属性。正文没有明确写出改造、替换、伪装、损毁导致变化时，不得改变
- owner、location、status、appearance 可随正文实际变化更新；lastSeenChapter 必须写当前章，firstSeenChapter 必须保留首次出现章
- 物品承担伏笔时，把对应 hookId 写入 linkedHookIds；即使隔几十章未出现，也不得删除对象记录
- 如果正文明确改变了稳定属性，必须填写 attributeChangeReason，逐字说明正文中的变化动作；没有正文证据时留空`;
...
\`\`\`json
{
  ...
  "objectOps": {
    "upsert": [
      {
        "objectId": "mentor-seal-token",
        "name": "师门旧令牌",
        "aliases": ["腰牌"],
        "material": "木",
        "inscription": "镇妖司",
        "appearance": "边缘磨损，有一道旧缺口",
        "owner": "玄微",
        "location": "玄微腰间",
        "status": "随身持有",
        "firstSeenChapter": 1,
        "lastSeenChapter": 12,
        "linkedHookIds": ["mentor-past"],
        "notes": "身份伏笔载体"
      }
    ]
  },
...
export function buildSettlerUserPrompt(params: {
  ...
  const objectLedgerBlock = params.objectLedger && params.objectLedger !== "(文件尚未创建)"
    ? `\n## 当前持久对象账本（跨章硬事实）\n${params.objectLedger}\n`
    : "";
