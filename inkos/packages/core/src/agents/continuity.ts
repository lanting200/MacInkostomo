...
const BLOCKING_AUDIT_PATTERNS = [
  /时间(?:线)?(?:错位|倒流|断裂|矛盾|不连续)/i,
  /timeline\s+(?:break|regression|contradiction|inconsistency)/i,
  /元叙事|编剧旁白|知识库污染|出戏/i,
  /meta[-\s]?narration|authorial aside|knowledge-base pollution/i,
  /设定冲突|人设矛盾|未授权(?:能力|外挂|金手指)|金手指失衡/i,
  /lore conflict|character contradiction|unauthorized (?:ability|cheat)|imbalanced cheat/i,
];

function shouldPromoteAuditIssue(issue: AuditIssue): boolean {
  if (issue.severity === "critical") return false;
  const haystack = `${issue.category}\n${issue.description}\n${issue.suggestion}`;
  return BLOCKING_AUDIT_PATTERNS.some((pattern) => pattern.test(haystack));
}

function normalizeAuditResult(result: AuditResult): AuditResult {
  const issues = result.issues.map((issue) =>
    shouldPromoteAuditIssue(issue)
      ? {
          ...issue,
          severity: "critical" as const,
          repairScope: issue.repairScope ?? "structural",
          suggestion: issue.suggestion
            ? `${issue.suggestion}（该问题已自动升级为 critical，必须修复后才能通过。）`
            : "该问题已自动升级为 critical，必须修复后才能通过。",
        }
      : issue,
  );
  ...
  return {
    ...result,
    passed: hasCritical ? false : result.passed,
    issues,
  };
}
...
export class ContinuityAuditor extends BaseAgent {
  ...
  async auditChapter(
    ...
    const [diskCurrentState, diskLedger, objectLedger, diskHooks, styleGuideRaw, subplotBoard, emotionalArcs, characterMatrix, chapterSummaries, parentCanon, fanficCanon, volumeOutline] =
      await Promise.all([
        ...
        this.readFileSafe(join(bookDir, "story/object_ledger.md")),
...
Hard fail routing: timeline regression/contradiction across adjacent chapters, meta-narration inside chapter prose, knowledge-base pollution (e.g. characters/narration referring to "previous chapter", "this chapter", "reader", "plot"), unauthorized new abilities/cheats, and lore/personality contradictions MUST be severity "critical", repair_scope "structural", and passed=false. These are not cosmetic issues.

Persistent-object continuity is a mandatory first-pass check. Before scoring anything else, compare every recurring physical object against both the previous chapter and the persistent object ledger. Match identity by owner, narrative function, unique marks, linked hook, and context — not by generic noun alone. For the same object, unexplained changes to material, inscription, shape, quantity, owner, location, damage, or possession are critical lore conflicts. A gap of many chapters does not weaken this rule. Quote the conflicting old and new facts in the issue. Do not merge genuinely distinct objects merely because both are called token, ring, sword, key, or similar.
...
硬失败路由：相邻章节时间倒流/时间矛盾、正文出现元叙事、知识库污染（例如角色或叙述提到“上一章/本章/读者/剧情/作者”等）、未授权新增能力/金手指、设定或人设冲突，必须标 severity="critical"、repair_scope="structural"、passed=false。这些不是文笔小问题。

持久物品连续性是必须最先执行的检查。评分前逐件对照上一章全文和持久对象账本，按持有者、叙事功能、独特损痕、关联伏笔和上下文判断是否为同一物品，不能只按“令牌/戒指/剑/钥匙”等泛称判断。同一物品的材质、刻字/纹样、形状、数量、持有者、位置、损坏和持有状态若无正文解释发生变化，必须标 critical 设定冲突并引用新旧事实。即使物品隔了很多章没出现，这条规则仍然生效；也不要把两件不同物品因为泛称相同而误判为同一件。
...
综合评分，不要因为单一小问题大幅拉低分数。`;
    ...
    const ledgerBlock = gp.numericalSystem
      ? isEnglish
        ...
        : `\n## 资源账本\n${ledger}`
      : "";
    const objectLedgerBlock = objectLedger !== "(文件不存在)"
      ? isEnglish
        ? `\n## Persistent Object Ledger (cross-chapter hard facts)\n${objectLedger}\n`
        : `\n## 持久对象账本（跨章硬事实）\n${objectLedger}\n`
      : "";
