...
export function validatePostWrite(
  ...
  if (bookRules?.prohibitions) {
    for (const prohibition of bookRules.prohibitions) {
      ...
      const quotedTerms = extractQuotedProhibitionTerms(prohibition);
      for (const term of quotedTerms) {
        if (content.includes(term)) {
          violations.push({
            rule: "本书禁忌",
            severity: "error",
            description: `正文触犯本书禁忌短语："${term}"（来自规则：${prohibition}）`,
            suggestion: "删除或改写该短语，并按本书规则改成有铺垫、有代价、符合角色弧线的表达。",
          });
        }
      }
    }
  ...
  // 13. Narrative-person adherence (#290). A first-person book must read as first
...
function extractQuotedProhibitionTerms(prohibition: string): string[] {
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(prohibition)) !== null) {
...
export function detectTemporalContinuityBreak(
  currentContent: string,
  recentChaptersContent: string,
  language: "zh" | "en" = "zh",
): ReadonlyArray<PostWriteViolation> {
  if (!recentChaptersContent || recentChaptersContent.length < 100) return [];
  const isEnglish = language === "en";
  const previousTail = recentChaptersContent.slice(-3000);
  const currentHead = currentContent.slice(0, 1800);
  const currentAll = currentContent;
  const violations: PostWriteViolation[] = [];

  if (!isEnglish) {
    const previousAlreadyMorning = /(鸡叫第三遍|晨光|清晨|天光已亮|太阳|朝阳|日头|天色已亮)/.test(previousTail);
    const currentRegressesToPreDawn = /(报晓前|第一声鸣|天边刚泛起鱼肚白|鱼肚白|破晓前|黎明前|天还没亮)/.test(currentAll);
    if (previousAlreadyMorning && currentRegressesToPreDawn) {
      violations.push({
        rule: "跨章时间线",
        severity: "error",
        description: "上一章已进入清晨/晨光时段，本章又回到报晓前/鱼肚白/天未亮，造成相邻章节时间倒流。",
        suggestion: "统一时间线：若上一章已经清晨，本章只能写“天已亮/稍后/当日下山”，不得写“明早/报晓前/鱼肚白”。",
      });
    }

    if (/第\s*\d+\s*章/.test(currentHead)) {
      // 标题允许出现第N章；只检查正文开头以外会由章节号指称规则处理。
    }

    const currentSchedulesTomorrow = /(明早|明日一早|明天一早|明日天亮|明早再)/.test(currentAll);
    const currentImmediateContinuation = /(刚贴上|刚从|刚才|门外半枚|那撮|那根|门槛|仍|还|余火|火光刚)/.test(currentHead);
    if (currentImmediateContinuation && currentSchedulesTomorrow && previousAlreadyMorning) {
      violations.push({
        rule: "跨章行动承接",
        severity: "error",
        description: "本章开头承接上一章即时动作，却又把后续行动推到“明早”，与已发生的清晨时间和紧迫目标冲突。",
        suggestion: "把“明早/明日”改为“稍后/天亮后/吃过符水就下山”，并同步章尾时间。",
      });
    }
  } else {
    const previousAlreadyMorning = /\b(third cockcrow|morning light|dawn had broken|sunrise|sunlight)\b/i.test(previousTail);
    const currentRegressesToPreDawn = /\b(before dawn|pre-dawn|first crow|first light|the sky was just paling)\b/i.test(currentAll);
    if (previousAlreadyMorning && currentRegressesToPreDawn) {
      violations.push({
        rule: "timeline-continuity",
        severity: "error",
        description: "The previous chapter had already reached morning/dawn, but this chapter regresses to pre-dawn timing.",
        suggestion: "Keep adjacent chapters on one timeline; use 'later that morning' or 'after daybreak' instead of pre-dawn markers.",
      });
    }
  }
