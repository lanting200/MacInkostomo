...
export class ReviserAgent extends BaseAgent {
  ...
  async reviseChapter(
    ...
    const [currentState, ledger, objectLedger, hooks, styleGuideRaw, volumeOutline, storyBible, characterMatrix, chapterSummaries, parentCanon, fanficCanon] = await Promise.all([
    ...
    const protagonistBlock = bookRules?.protagonist
      ...
      : "";
    // Revision length is advisory: preserve the complete scene over a numeric target.
    const lengthGuardrail = mode !== "auto" && options?.lengthSpec
      ? (isEnglish
          ? "\n8. Length is advisory only. Preserve necessary scenes and context; do not cut material merely to meet a numeric target."
          : "\n8. 字数仅作参考。优先保留必要场景和上下文，不要为了凑字数而删减正文。")
      : "";
    ...
    const reducedControlBlock = options?.contextPackage && options.ruleStack
      ...
      : "";
    // Revision requests do not use length as an acceptance condition.
    const lengthGuidanceBlock = mode !== "auto" && options?.lengthSpec
      ? "\n## 字数说明\n字数仅作编辑参考，不作为本次修订的验收条件。优先完成审稿意见并保留必要情节、因果和上下文；不要为压缩篇幅删掉完整段落。\n"
      : "";
...
${chapterContent}`;
  ...
  private buildAutoSystemPrompt(params: {
    ...
    // lengthGuardrail intentionally not used in auto mode — auto guidance is embedded in REVISED_CONTENT description
    const en = resolvedLanguage === "en";
    const ledgerSection = gp.numericalSystem
      ? (en ? "\n=== UPDATED_LEDGER ===\n(Full updated resource ledger)" : "\n=== UPDATED_LEDGER ===\n(更新后的完整资源账本)")
      : "";
    const rewriteLengthGuidance = lengthSpec
      ? (en
          ? `\n  Length guidance: ${lengthSpec.target} characters is only a reference. Preserve complete scenes and context; length never decides whether this revision is accepted.`
          : `\n  字数说明：目标 ${lengthSpec.target} 字仅作参考。保留完整场景和上下文，字数不作为本次修订是否通过的条件。`)
      : "";
