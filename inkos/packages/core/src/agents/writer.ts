...
export class WriterAgent extends BaseAgent {
  ...
  async writeChapter(input: WriteChapterInput): Promise<WriteChapterOutput> {
    ...
    const [
      storyBible, volumeOutline, styleGuide, currentState, ledger, objectLedger, hooks,
      ...
      parentCanon, fanficCanonRaw,
        ...
        this.readFileOrDefault(join(bookDir, "story/object_ledger.md")),
    ...
    const ruleViolations = [
      ...
      ...detectTemporalContinuityBreak(surfaceNormalizedContent, recentChapters, resolvedLanguage),
  ...
  async settleChapterState(input: SettleChapterStateInput): Promise<WriteChapterOutput> {
    const [
      ...
      this.readFileOrDefault(join(input.bookDir, "story/object_ledger.md")),
  ...
  private async settle(params: {
    ...
    const settlerUser = buildSettlerUserPrompt({
      ...
      objectLedger: this.capLegacyContext("object_ledger", params.objectLedger, LEGACY_WRITER_CONTEXT_BUDGET.objectLedger),
  ...
  private buildUserPrompt(params: {
    ...
    const objectLedger = this.capLegacyContext(
      "object_ledger",
      params.objectLedger ?? "",
      LEGACY_WRITER_CONTEXT_BUDGET.objectLedger,
    );
    ...
    const ledgerBlock = ledger
      ? `\n## 资源账本\n${ledger}\n`
      : "";
    const objectLedgerBlock = objectLedger && objectLedger !== "(文件尚未创建)"
      ? `\n## 持久对象账本（跨章硬事实）\n${objectLedger}\n`
      : "";
...
- 先输出写作自检表，再写正文
  ...
  private buildGovernedUserPrompt(params: {
    ...
    const recentChapters = this.capLegacyContext(
      "recent_chapters",
    ...
    const continuityBlock = language === "en"
      ? `## Adjacent chapter full text (hard continuity)\n${recentChapters || "(first chapter)"}\n\n## Persistent object ledger (cross-chapter hard facts)\n${objectLedger && objectLedger !== "(文件尚未创建)" ? objectLedger : "(none yet)"}\n`
      : `## 上一章全文（相邻章硬连续性）\n${recentChapters || "（第一章，无前文）"}\n\n## 持久对象账本（跨章硬事实）\n${objectLedger && objectLedger !== "(文件尚未创建)" ? objectLedger : "（尚无记录）"}\n`;
  ...
  private async loadRecentChapters(
    ...
    try {
      ...
      const mdFiles = files
        ...
        .filter((f) => {
          const number = Number.parseInt(f.slice(0, 4), 10);
          return Number.isFinite(number) && number < currentChapter;
        })
        .sort()
        .slice(-count);
  ...
  private async resolveRuntimeStateArtifactsForOutput(
    ...
    if (
      ...
      return {
        ...
        objectLedgerMarkdown: renderObjectLedgerProjection(
          output.runtimeStateSnapshot.objects ?? { objects: [] },
          language,
        ),
      };
    }
