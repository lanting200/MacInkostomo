...
export function renderObjectLedgerProjection(
  state: PersistentObjectsState,
  language: "zh" | "en" = "zh",
): string {
  const title = language === "en" ? "# Persistent Object Ledger" : "# 持久对象账本";
  const headers = language === "en"
    ? [
        "| object_id | name / aliases | material | inscription | appearance | owner | location | status | first_seen | last_seen | linked_hooks | notes |",
        "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
      ]
    : [
        "| object_id | 名称/别名 | 材质 | 刻字/纹样 | 外观特征 | 持有者 | 位置 | 状态 | 首次出现 | 最近出现 | 关联伏笔 | 备注 |",
        "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
      ];
  const rows = state.objects.map((object) => `| ${[
    object.objectId,
    [object.name, ...object.aliases].filter(Boolean).join(" / "),
    object.material,
    object.inscription,
    object.appearance,
    object.owner,
    object.location,
    object.status,
    object.firstSeenChapter,
    object.lastSeenChapter,
    object.linkedHookIds.join(", "),
    object.notes,
