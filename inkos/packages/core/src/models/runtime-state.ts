...
export const PersistentObjectRecordSchema = z.object({
  objectId: z.string().min(1),
  name: z.string().min(1),
  aliases: z.array(z.string().min(1)).default([]),
  material: z.string().default(""),
  inscription: z.string().default(""),
  ...
  linkedHookIds: z.array(z.string().min(1)).default([]),
  notes: z.string().default(""),
  attributeChangeReason: z.string().optional(),
});

export type PersistentObjectRecord = z.infer<typeof PersistentObjectRecordSchema>;

export const PersistentObjectsStateSchema = z.object({
  objects: z.array(PersistentObjectRecordSchema).default([]),
});

export type PersistentObjectsState = z.infer<typeof PersistentObjectsStateSchema>;

export const ObjectOpsSchema = z.object({
  upsert: z.array(PersistentObjectRecordSchema).default([]),
});

export type ObjectOps = z.infer<typeof ObjectOpsSchema>;
