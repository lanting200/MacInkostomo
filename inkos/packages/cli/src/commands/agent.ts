...
import { buildPipelineConfig, loadConfig, createClient, createTextDeltaEmitter, findProjectRoot, resolveBookId, resolveContext, log, logError } from "../utils.js";
...
export const agentCommand = new Command("agent")
  ...
  .action(async (instruction: string, opts) => {
    try {
      ...
      const emitTextDelta = createTextDeltaEmitter();
      ...
      const result = await runAgentSession(
        {
          ...
          onEvent: (event: any) => {
            if (event.type === "message_update" && event.assistantMessageEvent?.type === "text_delta") {
              emitTextDelta?.(event.assistantMessageEvent.delta);
            }
          },
        },
        fullInstruction,
