    ...
    try {
      ...
      for (const id of bookIds) {
        ...
        const failed = index.filter(
          (ch) => ch.status === "audit-failed" || ch.status === "rejected",
        ).length;
        ...
        booksData.push({
          ...
          ...(opts.chapters ? {
            chapterList: index.map((ch) => ({
              ...
              ...(ch.status === "audit-failed" || ch.status === "rejected" || ch.status === "state-degraded"
          ...
          if (opts.chapters && index.length > 0) {
            ...
            for (const ch of index) {
              const icon = ch.status === "approved"
                ? "+"
                : ch.status === "audit-failed" || ch.status === "rejected"
                  ? "!"
                  : ch.status === "state-degraded"
                    ? "x"
                    : "~";
              log(`    [${icon}] Ch.${ch.number} "${ch.title}" | ${formatLengthCount(ch.wordCount, countingMode)} | ${ch.status}`);
              if ((ch.status === "audit-failed" || ch.status === "rejected" || ch.status === "state-degraded") && ch.auditIssues.length > 0) {
