...
const WRITER_TEXT_EVENT_PREFIX = "@@INKOS_TEXT@@";
const WRITER_TEXT_EVENT_MAX_CHARS = 1_600;
const WRITER_TEXT_EVENT_DELAY_MS = 300;

export function createTextDeltaEmitter(): ((text: string) => void) | undefined {
  if (process.env.INKOS_EMIT_WRITER_TEXT !== "true") return undefined;
  ...
  const flush = () => {
    ...
    const text = buffered;
    buffered = "";
    process.stdout.write(`${WRITER_TEXT_EVENT_PREFIX}${JSON.stringify({ type: "writer_delta", text })}\n`);
  };

  return (text: string) => {
    if (!text) return;
    buffered += text;
    if (buffered.length >= WRITER_TEXT_EVENT_MAX_CHARS) {
      flush();
      return;
    }
    if (!timer) {
      timer = setTimeout(flush, WRITER_TEXT_EVENT_DELAY_MS);
    }
  };
}
...
export function buildPipelineConfig(
  ...
  const onWriterTextDelta = createTextDeltaEmitter();
