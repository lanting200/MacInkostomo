...
- 写法内核在 Swift 侧固定生效，覆盖场景纪律、入场方式、信息节奏、对话承载、场面完成度、视角、时间尺度、章末处理、篇幅来源和去 AI 腔；前三章附加开篇规则，限制单章线索数量并禁止金手指在本章内完成完整闭环。
...
- 生成与修订在送审前先过本地确定性校验：清单式条目铺陈、叙述边界上的“第28天/雨季第28天”式绝对纪日、总结或淡出式收尾、以及叙述里反复出现的「不是……而是……」否定修正句、空气凝固/心中暗道套喻和「极其」刷词直接退稿；对白里角色这样说、以及叙述偶发一次可以保留。“第二天一早”等相对过渡和角色在对话、台账、物资心算中引用日期同样保留。前三章作为开篇段整体至少建立一次核心能力异常锚点，前章已经建立后不要求后续章重复，且节拍卡的禁止清单优先于重复锚点。这些规则与审核模型的同类 `[hard][prose]` 规则互为兜底。
...
- 本地校验失败稿按默认意见再次提交时先重新校验并复审原正文，仍未通过才进入全文自动修改；正文、排期与篇幅问题最多执行 `maxAutoRevisionRounds` 轮（默认 3）重写+复审。两类情况提前终止而不耗尽轮次：模型两次返回逐字相同的正文（第一次会抬高温度并给出强化指令重试），以及模型请求配置错误或端点实际返回 4xx（429 除外，那已由传输层重试过）。本地校验虽然也使用 409/422 向 UI 表达错误类型，但不会再被误判成端点拒绝；它的具体反馈会进入下一轮修改指令。模型停滞时，章节记录的失败原因是当前真正的阻断项，不是"输出与原文完全相同"这类机制信息。
...
- 上下文窗口（`contextWindow`，默认 200000 token）：按剩余输入预算裁剪章节提示词里的设定上下文。
- 最大输出（`maxTokens`，默认 16384 token）：写入请求的 `max_tokens`。配置文件缺省时也按 16384 发送，不再省略该字段。空回复仍会按现有规则加倍重试，上限 65536。

## 原著正典抽取

同人创作只需要客户提供两样东西：一份原著 txt 和一段作者设定文本。

- `importDerivativeSource` 导入原著：识别编码、切分章节并建立 SQLite/FTS 词法索引；准备流程再按创建时的意图完成端侧语义向量。选择文件时立即把原始字节拷进 `data/pending-sources/`，创建小说期间不再依赖用户路径。切章会去掉 txt 转储常见的 `====` / `【】` 装饰，并把文首目录与后文同序号章节合并。导入过程通过进度回调刷新横幅（识别编码、切章、建索引、提交）。已有同人书可在设置页更换原著；导入失败的横幅提供“重新选择原著”。
- `extractDerivativeCanon` 把原著抽成正典：按字数预算分批（默认每批 1.5 万字符）交给原著抽取模型。滑动窗口内最多 3 个相邻批次同时等待模型，但结果仍严格按原著顺序逐批归一化、合并进连续性投影的 `baseContinuity` 并写入原子检查点；窗口内稍后启动的批次可能看不到仍在途的实体，最终实体 ID 由按名称归并的提交阶段统一确定。数字章节之前保留的 index 0 卷首/序章也会纳入抽取，并以独立状态位检查点化；进度写在 `source/canon-progress.json`，每批落一次盘，中断后可续跑，旧进度会从已完成批次恢复序章状态。某一批失败只保留它之前连续完成的批次，不会让后发响应越过缺口推进游标。换了原著文件会同时清掉旧进度和旧 `baseContinuity`，但保留作者 overlay 与已审核章节贡献。
- `extractDerivativeSettingsOverlay` 把作者设定文本写进 `manualOverlay`。这一层在投影时最后应用且允许覆盖不可变字段，所以设定文本与原著冲突时以设定文本为准。
- 原著章号会统一改写成衍生作自己的章号，原始出处保留在 `markers` 里（`source-chapter-N`）。抽取结果若把同一人物同时写进知情与禁知列表，会从禁知侧去掉重叠项并记录 `canon.knowledge.knower_overlap`，避免一整批正典被校验拦住；章节 Delta 仍按原规则拒绝重叠。
...
导入与抽取由新建小说流程调用（`WorkspaceModel.prepareDerivativeSource`）。每个批次完成原子 checkpoint 后，核心通过类型化进度回调立即刷新工作区顶部的章数、实体数与时间线事件数；长时间抽取期间不必等待整个 `extractDerivativeCanon` 返回。`embedDerivativeSource` 同样按批提交向量并回传覆盖率，横幅在整本索引完成前就能从已写入条数往前走，而不是一直停在 0。`generateChapter`、`reviseChapter` 和 `approveChapter` 都会先确认正典、作者 overlay、所请求的语义索引以及可解析的时间锚点已经准备完成；准备不完整时不会启动写作任务或批准章节。创建时填写的简称（如「克莱恩穿越」）会按尾两字匹配已抽取事件，并优先在原著前 80 章里绑定，避免对上后文同词揭示。
