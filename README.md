...
- 节拍卡按卷内每 10 章一批生成，写入 `story/runtime/chapter-beats.json`，包含本章目标、入场点、场景、必须发生的事件、必须出现的挫折、**禁止提前出现或提前解决的内容**、时间跨度、新增具名人物上限和章末钩子。批次规划是最慢的一步，因此当前批次剩余 3 章以内时后台自动预排下一批，不再阻塞章节生成。
